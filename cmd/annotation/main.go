...
func main() {
	...
	e.GET("/ws", handlers.NewAnnotationHandler().HandleAnnotation)
	e.GET("/ws/remain-count", handlers.NewRemainImageCountHandler().HandleRemainImageCount)
	e.GET("/ws/predict-remain-count", handlers.NewPredictedRemainImageCountHandler().HandlePredictedRemainImageCount)
	e.GET("/ws/check", handlers.NewAnnotatedDataCheckHandler().HandleAnnotatedDataCheck)
	e.GET("/ws/ai", handlers.NewPredictedDataHandler().HandlePredictedData)
